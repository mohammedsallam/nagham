              ...
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><img src="images/{{$type->imageUrl}} " style="width: 50px; height: 50px"></td>
                    <td><a href="{{route('contentIndex', $type-> id)}}" class="btn btn-success">view Content</a></td>

                    <td>
                      <div class="row">
                        <form class="mr-1 " action="/Deletetype/{{$type->id}}" method="POST">
                          @csrf
                          @method('DELETE')
                          <button class="btn btn-danger" type="submit">Delete</button>
                        </form>

                        <button class="btn btn-info" data-id="{{$type->id}}" data-name="{{$type->name}}" data-information="{{$type->information }}" data-imageType="{{asset('images/'.$type->imageUrl)}}" data-toggle="modal" data-target="#ModalEdit" data-whatever="@getbootstrap">Edit</button>
                      </div>
                      <div class="modal fade" id="ModalEdit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                          <form method="POST" action="/Edittype" enct ype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            <div class="modal-content">
                              <div class="modal-header">
...
<script>
  ...
  $('#ModalEdit').on('show.bs.modal', function(event) {
    ...
    var imageUrl = btnEdit.data('imageType');
